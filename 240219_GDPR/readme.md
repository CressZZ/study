...
# Activate Google signals to enhance remarketing and reporting - GoogleAds 와 연동
구글 시그널 데이트를 Ads에서 활용하려면 GoogleAds 와 GA 를 연동해야 한다고 함. 

## https://support.google.com/analytics/answer/9039086?hl=ko&ref_topic=7668613&sjid=5689117008349390590-AP
Google Ads로 교차 기기 전환 내보내기
교차 기기 전환은 사용자가 한 기기에서 광고를 클릭한 다음 다른 기기에서 전환을 완료(전자상거래, 목표 달성)할 때 발생합니다. 예를 들어 사용자가 아침 출근 중에 모바일 웹브라우저에서 Google Ads 광고를 클릭한 후 저녁에 집에서 데스크톱 브라우저에서 해당 물품을 구매하는 경우 교차 기기 전환이 발생합니다.

애널리틱스에서는 Google 계정에 로그인하여 광고 개인 최적화를 사용 설정한 사용자의 교차 기기 전환 데이터를 토대로 사이트의 전자상거래 및 목표 달성에 대해 별도의 맞춤 모델을 만듭니다. 이 모델에서는 로그인 데이터를 이용해 모든 사이트 사용자의 교차 기기 전환 측정항목을 추정합니다. 그런 다음 애널리틱스에서 모델링된 교차 기기 전환과 동일 기기 전환을 Google Ads로 내보내 입찰에 사용할 수 있습니다.

교차 기기 전환 행동에 대한 이해도가 높아질수록 해당 데이터를 광고 게재위치 및 입찰의 기초 자료로서 더욱 가치있게 사용할 수 있습니다. 예를 들어 교차 기기 > 기기 경로 보고서에 모바일 > 데스크톱 경로의 전환율이 가장 높은 것으로 나타난 경우, 더 많은 상위 유입경로 광고를 휴대기기에 배치하고 해당 광고에 대해 더 높은 입찰가를 설정할 수 있습니다.

전자상거래 및 목표에 대해 자세히 알아보기
이점
교차 기기 행동에 대한 정보
교차 기기 전환 데이터를 연결된 Google Ads 계정에 자동으로 내보내기
교차 기기 전환 데이터를 Google Ads 입찰에 포함
요건
필수 요건
애널리틱스와 Google Ads 계정을 연결해야 합니다.
애널리틱스 속성에서 매월 10,000회 이상의 전환이 발생해야 합니다(전자상거래 또는 목표 달성).
Google 신호 데이터를 활성화해야 합니다.
이 도움말에 제시된 대로 Google Ads 전환 액션을 입찰에 포함해야 합니다.
참고사항
Google Ads에서는 가져온 애널리틱스 전환 데이터를 입찰에 사용하고 있습니다.

## https://support.google.com/analytics/answer/1033961#zippy=%2C%EC%9D%B4-%EB%8F%84%EC%9B%80%EB%A7%90%EC%97%90-%EB%82%98%EC%99%80-%EC%9E%88%EB%8A%94-%EB%82%B4%EC%9A%A9%EC%9D%80-%EB%8B%A4%EC%9D%8C%EA%B3%BC-%EA%B0%99%EC%8A%B5%EB%8B%88%EB%8B%A4


 ## Universal Analytics properties > 보고 및 분석광고 > Google Ads 통합 > 애널리틱스 목표 및 거래를 Google Ads 전환 추적으로 가져오기


# 동의모드 구현 예
## 순서 https://developers.google.com/tag-platform/security/guides/consent?sjid=15161908206018094414-AP&hl=ko#gtag.js
여기에서는 코드의 순서가 중요합니다. 동의 코드가 잘못된 순서로 호출되면 동의 기본값이 작동하지 않습니다. 비즈니스 요구사항에 따라 구체적인 내용이 달라질 수 있지만 일반적으로 코드는 다음과 같은 순서로 실행해야 합니다.
Google 태그를 로드합니다. 이 코드는 기본 스니펫 코드입니다. 기본 스니펫은 gtag('consent', 'default', ...) 호출을 포함하도록 업데이트해야 합니다(아래 참고). 기본값을 설정하지 않으면 모든 태그 지정 기능이 사용 설정됩니다.
동의 솔루션을 로드합니다. 동의 솔루션이 비동기식으로 로드되는 경우 이 과정이 올바른 순서로 실행되는지 확인하는 방법은 비동기 동의 관리 플랫폼과 통합을 참고하세요.
동의 솔루션에 의해 처리되지 않으면 사용자가 동의한다고 명시한 후 gtag('consent', 'update', ...)를 호출하세요.