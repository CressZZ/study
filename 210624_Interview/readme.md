...
## 그럼 백엔드 업무도 하신건가요? 백엔드 담당자가 있었다면 업무 분배는 어떻게 하셨나요?
- 혼자 했어요!
- 백엔드는 서버 관리? 랑 API 만 만들었어요! 페이지 라우팅 같은건 제가 했어요!

## 오호 그럼 노드서버 구축 과정에서 생겼던 문제중 큰 문제는 무엇이 있었나요?
- 서버가 뻗어서 캐시를 적용했더니 안뻗었어요!

## 오호 그럼 AWS 를쓰신건가요?  아니면 Azure?
- AWS 요!!

## 그렇군요 그럼 nginx 도 쓰셨나요? 설정이나 구축도 해보셨나요?
- 아니요! 
- 네! 제가 다했어요!

# 프로젝트 세팅할때 NPM? yarn? 왜 그걸 쓰셨죠?
- NPM밖에 잘 몰라요
- yarn 이 빨라요! 

# 타입스크립트 써보셨나요? 웹펙 돌릴때 어떤 로더로 타입스크립트를 컴파일 하셨나요?
- 네!!!! ts-loader 요! babel-loader 요! => 두개의 차이점을 제가 잘 모릅...

# css 는 사스를 쓰는지 네이밍 규칙 ? 어떤 방법론을 선호 하시나요? BEM같은...
- 사스를 써요! 규칙은 회사 규칙을 따릅니다! BEM 을 선호 합니다

#  Github의 캐릭터는 문어일까요? 고양이 일가요? 어떤 동물 인가요? (함정질문)
- octocat!
...
# 저희가 프론트엔드 면접이라 클로저를 안여쭤 볼수가 없어요 ㅎㅎㅎㅎ 클로저가 무엇인가요?
...
# babel에서 주로 쓰시는 플러그인이 있으신가요?
- 네 뭐 여러가지 "@babel/plugin-proposal-class-properties", 등 씁니다. 
- 안씁니다. preset/env 로 충분 하죠
...
# 프로미스란 무엇인가? 프로미스를 어디에 사용하는가? 우리함께 프로미스로 코드를 짜봅시다. 
Promise 객체는 비동기 작업이 맞이할 미래의 완료 또는 실패와 그 결과 값을 나타냅니다.

```js

doAfter10Sec().then(sayDone);

function sayDone(){
	console.log('done')
}

function doAfter10Sec(){
	// ???
}
